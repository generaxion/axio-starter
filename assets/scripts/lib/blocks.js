...
const tagFirstAndLastBlock = () => {
  ...
  if (blocks) {
    for (let i = 0; i < blocks.childNodes.length; i++) {
      if (blocks.childNodes[i].nodeType !== 3 && blocks.childNodes[i].classList !== "undefined") {
  ...
  if (innerBlocks.length) {
    for (let i = 0; i < innerBlocks.length; i++) {
      for (let j = 0; j < innerBlocks[i].childNodes.length; j++) {
        if (innerBlocks[i].childNodes[j].nodeType !== 3 && innerBlocks[i].childNodes[j].classList !== "undefined") {
